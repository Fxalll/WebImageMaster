...
(function () {
  ...
  async function safeStorageGet(keys) {
    try {
      if (!isExtensionAlive()) {
        console.log("扩展已失效，跳过读取存储");
        return {}; // 返回空对象，让代码继续走默认值
      }
      return await chrome.storage.local.get(keys);
    } catch (e) {
      console.log("读取存储失败（扩展可能已失效）:", e.message);
      return {};
    }
  }
  ...
  async function safeStorageSet(items) {
    try {
      if (!isExtensionAlive()) {
        console.log("扩展已失效，跳过保存存储");
        return; // 直接返回，不抛错
      }
      await chrome.storage.local.set(items);
    } catch (e) {
      console.log("保存存储失败（扩展可能已失效）:", e.message);
    }
  }

  function getGlobalConfig(key) {
    if (_globalSettingsCache && _globalSettingsCache[key] !== undefined) {
      return _globalSettingsCache[key];
    }
    return defaultConfig[key];
  }

  async function setGlobalConfig(key, value) {
    if (!_globalSettingsCache) _globalSettingsCache = {};
    _globalSettingsCache[key] = value;
    await safeStorageSet({
      nopic_global_settings: _globalSettingsCache,
    });
  }

  // 启动时预加载缓存，加载完成后刷新一次内存变量
  async function preloadGlobalSettings() {
    const result = await safeStorageGet("nopic_global_settings");
    _globalSettingsCache = result.nopic_global_settings || {};

    // 用真实存储值覆盖默认值
    statsRangeConfig = getGlobalConfig("statsRange");
    configs.time = getGlobalConfig("displayTime");
    configs.seconds = getGlobalConfig("displaySeconds");
    configs.count = getGlobalConfig("displayCount");
    configs.total = getGlobalConfig("displayTotal");
    configs.percent = getGlobalConfig("displayPercent");
    configs.host = getGlobalConfig("displayHost");
    configs.memory = getGlobalConfig("displayMemory");
    configs.loadTime = getGlobalConfig("displayLoadTime");
    configs.resCount = getGlobalConfig("displayResCount");
    configs.mouseDistance = getGlobalConfig("displayMouseDistance");
    configs.clickCount = getGlobalConfig("displayClickCount");
    configs.scrollDist = getGlobalConfig("displayScrollDist");
    configs.readChars = getGlobalConfig("displayReadChars");
    configs.adCount = getGlobalConfig("displayAdCount");
  ...
  // 先给默认值，等 preloadGlobalSettings() 执行后会自动刷新为真实存储值
  let statsRangeConfig = defaultConfig.statsRange;

  let configs = {
    time: defaultConfig.displayTime,
    seconds: defaultConfig.displaySeconds,
    count: defaultConfig.displayCount,
    total: defaultConfig.displayTotal,
    percent: defaultConfig.displayPercent,
    host: defaultConfig.displayHost,
    memory: defaultConfig.displayMemory,
    loadTime: defaultConfig.displayLoadTime,
    resCount: defaultConfig.displayResCount,
    mouseDistance: defaultConfig.displayMouseDistance,
    clickCount: defaultConfig.displayClickCount,
    scrollDist: defaultConfig.displayScrollDist,
    readChars: defaultConfig.displayReadChars,
    adCount: defaultConfig.displayAdCount,
  };
  ...
  let createControlButton = function (el) {
    ...
    // 防抖触发灯旋转动画
    let debounceTriggerSpinner = (function () {
      let timer = null;
      return function () {
  ...
  async function loadGlobalStats() {
    try {
      const result = await safeStorageGet("nopic_global_stats_v1");
      const saved = result.nopic_global_stats_v1;
      if (saved && saved.data) {
        const now = new Date();
  ...
  async function saveGlobalStats() {
    try {
      // 先检查扩展是否还活着，如果死了就直接返回，不抛错
      if (!isExtensionAlive()) {
        console.log("扩展上下文已失效，统计数据保留在内存中，不写入存储");
        return;
      }

      const result = await safeStorageGet("nopic_global_stats_v1");
      const stored = result.nopic_global_stats_v1;
      let latestData =
        stored && stored.data
          ? stored.data
          : { today: { mouse: 0, click: 0, scroll: 0, read: 0 } };
      ...
      await safeStorageSet({
        nopic_global_stats_v1: { lastUpdate: Date.now(), data: latestData },
      });
    } catch (e) {
  ...
  function updateCharDensity() {
    ...
    // 关键修复：检查 document.body 是否存在
    if (!document.body) {
      console.log("document.body 不存在，跳过字符密度计算");
      return;
    }

    // 安全读取 innerText（某些元素可能不支持）
    const text = document.body.innerText || "";
    charDensity = text.replace(/\s/g, "").length / scrollH;
  }
  document.addEventListener("mousemove", (e) => {
...
`;
  ...
  displaySubmenu.addEventListener("click", (e) => {
    ...
    if (rangeBtn) {
      ...
      setGlobalConfig("statsRange", statsRangeConfig).catch(() => {});
      updateAllUI();
      updateContent();
      return;
    }
    ...
    if (item) {
      ...
      if (configs[mapKey] !== undefined) {
        ...
        setGlobalConfig(key, configs[mapKey]).catch(() => {});
      }
      updateAllUI();
      updateContent();
      return;
  ...
  (async function initExtension() {
    ...
    // 预加载完成后，如果本站在自动隐藏列表里，执行一次 imgHiden
    if (
